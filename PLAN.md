...
45. Done! 🎉
### Phase 8: Firebase Integration ✅ (Completed)
```
46. ✅ Install firebase_core, firebase_auth, cloud_firestore
47. ✅ Create Firebase project at console.firebase.google.com
48. ✅ Run `flutterfire configure` to link app
49. ✅ Create auth_service.dart (signUp, signIn, signOut, authStateChanges)
50. ✅ Create auth_provider.dart (Riverpod providers for auth state)
51. ✅ Create login_screen.dart (email + password form)
52. ✅ Create signup_screen.dart (account creation form)
53. ✅ Add phone number field to UserProfile model + ProfileSetupScreen
54. ✅ Add toJson() / fromJson() to UserProfile, HealthData, HealthReport
55. ✅ Update StorageService — save to Hive + Firestore simultaneously
56. ✅ Update app.dart — auth guard redirect + /login, /signup routes
57. ✅ Update dashboard_screen.dart — logout button
58. ✅ Update main.dart — Firebase.initializeApp()
59. ✅ Build & verify APK compiles successfully
```

### Phase 9: Auth Flow & UI Fixes (Next Up)
```
60. Add shared_preferences dependency
61. Fix SplashScreen: show disclaimer only on first launch, route by auth state
62. Fix DisclaimerScreen: save "accepted" flag, route to /login
63. Update ProfileSetupScreen: pre-fill form with existing data, show profile card
64. Add Firestore fetch methods to StorageService (fetchProfileFromCloud, etc.)
65. Add loadFromCloud() to profile_provider, health_data_provider, report_provider
66. Fetch cloud data on login in SplashScreen
67. Create email_verification_screen.dart (sendEmailVerification, check status)
68. Create forgot_password_screen.dart (sendPasswordResetEmail)
69. Add "Forgot Password?" link on login_screen
70. Add routes: /email-verification, /forgot-password
71. Clear local Hive boxes on logout
72. Add password visibility toggle on login + signup
73. Add loading spinners during Firestore sync
74. Add friendly empty states (dashboard, report history)
```

### Phase 9.5: Critical Data Entry Fixes
```
75. Update data_category_screen.dart to list all 12 medical categories
76. Create lib/utils/test_definitions.dart describing keys and units for all fields
77. Update data_entry_screen.dart to dynamically render ONLY fields that match selected categories
```

### Phase 10: New Features
```
78. PDF Report: Create pdf_report_service.dart (generate full-page PDF from HealthReport)
79. PDF Report: Add download + share buttons to report_screen.dart AppBar
80. Health Trends: Create trends_screen.dart with fl_chart line charts per vital
81. Health Trends: Add "Health Trends" card on dashboard, /trends route
82. Dark Mode: Create theme_provider.dart (persist preference to SharedPreferences)
83. Dark Mode: Define light + dark ThemeData in app.dart
84. Dark Mode: Add moon/sun toggle icon in dashboard AppBar
85. OCR Scanner: Add google_mlkit_text_recognition + image_picker dependencies
86. OCR Scanner: Create ocr_service.dart (on-device text recognition + lab value parsing)
87. OCR Scanner: Create ocr_review_screen.dart (editable extracted values)
88. OCR Scanner: Add "📷 Scan Lab Report" button on data_category_screen.dart
89. OCR Scanner: Add /ocr-review route
90. Final build & test all phases
```

---

## 22. Phase 8 — Firebase Integration (Details)

> [!TIP]
> **Status: ✅ COMPLETED** — Firebase Auth + Firestore are fully integrated.

### What Was Done
- **Authentication**: Email/password login and signup via Firebase Auth
- **Cloud Database**: User profiles, health data, and reports sync to Firestore under `users/{uid}/`
- **Routing Guard**: Unauthenticated users are redirected to `/login` automatically
- **Phone Number**: Added as an optional field on Profile Setup screen (stored in Firestore, NOT used for SMS/OTP)
...
users/
  {uid}/
    profile/
      current → { name, email, phone, age, gender, height, weight }
    health_data/
      {auto_id} → { category, testName, value, unit, date }
    reports/
      {reportId} → { date, highRisk[], moderateRisk[], lowRisk[], abnormalValues[] }
```
...
| `lib/screens/login_screen.dart` | Email + password login form |
...
| File | Change |
|------|--------|
| `lib/models/user_profile.dart` | Added `phone` field, `toJson()`, `fromJson()` |
| `lib/models/health_data.dart` | Added `toJson()`, `fromJson()` |
| `lib/models/report.dart` | Added `toJson()`, `fromJson()` |
| `lib/services/storage_service.dart` | Saves to Hive + Firestore simultaneously |
| `lib/screens/profile_setup_screen.dart` | Added phone number field |
| `lib/screens/dashboard_screen.dart` | Added logout button |
| `lib/app.dart` | Converted to ConsumerWidget, added auth guard + routes |
| `lib/main.dart` | Added `Firebase.initializeApp()` |

---

## 23. Phase 9 — Auth Flow & UI Fixes (Details)

### Problems Found During Testing

| # | Bug | Root Cause |
|---|-----|------------|
| 1 | Disclaimer reappears after every login | `SplashScreen` always routes to `/disclaimer` |
| 2 | Profile icon shows empty form | No "view profile" mode |
| 3 | Data lost after logout → re-login | Providers only read local Hive, never fetch from Firestore |
| 4 | No email verification | Firebase `sendEmailVerification()` not called |
| 5 | No "Forgot Password" | Missing screen and `sendPasswordResetEmail()` |

### Fix: App Flow
- **SplashScreen** → check `SharedPreferences.disclaimerAccepted` + `FirebaseAuth.currentUser`:
  - First launch → `/disclaimer` → `/login`
  - Logged out → `/login`
  - Logged in, no profile → `/profile-setup`
  - Logged in, has profile → `/dashboard`

### Fix: Profile View & Edit
- Pre-fill form with saved data from Firestore
- Show profile card at top (name, email, phone)
- Button says "Update Profile" when editing

### Fix: Data Persistence
- Add `fetchProfileFromCloud(uid)`, `fetchReportsFromCloud(uid)`, `fetchHealthDataFromCloud(uid)` to `StorageService`
- Call these in `SplashScreen` when a logged-in user is detected

### Email Verification (FREE)
- After signup → call `user.sendEmailVerification()`
- Show `email_verification_screen.dart` with "Resend" and "Continue" buttons

### Forgot Password (FREE)
- Add link on login screen → `forgot_password_screen.dart`
- Calls `FirebaseAuth.instance.sendPasswordResetEmail(email: ...)`
...
## 24. Phase 10 — New Features (Details)

### Feature A: PDF Report Download & Share

> Already installed: `pdf`, `printing`, `share_plus`

#### [NEW] `lib/services/pdf_report_service.dart`
- Generates a PDF from `HealthReport`:
  - Header: App name, user name, date
  - Body: High/Moderate/Low risk sections with colored markers
  - Clinical Findings list
  - Footer: Medical disclaimer

#### [MODIFY] `lib/screens/report_screen.dart`
- Add 📄 **Download PDF** button → preview/save via `printing`
- Add 📤 **Share** button → share via WhatsApp/Email using `share_plus`

---

### Feature B: Health Trends Dashboard

> Already installed: `fl_chart`

#### [NEW] `lib/screens/trends_screen.dart`
- Line charts for vitals over time:
...
- Reference range shown as green shaded band
- Tab bar to switch categories

#### [MODIFY] `lib/screens/dashboard_screen.dart`
- Add third card: "📊 Health Trends" → navigates to `/trends`
...
- Set `themeMode` from `themeProvider`

#### [MODIFY] `lib/screens/dashboard_screen.dart`
- Add 🌙/☀️ toggle icon in AppBar

---

### Feature D: OCR Lab Report Scanner

> [!IMPORTANT]
> Separate button — does NOT affect existing manual entry flow.

#### New Dependencies
```yaml
google_mlkit_text_recognition: ^0.14.0  # Free on-device OCR
image_picker: ^1.1.2                    # Camera/gallery access
```

#### [NEW] `lib/services/ocr_service.dart`
- On-device text recognition (no internet, 100% free)
- `parseLabValues(rawText)` extracts test name + value pairs using regex

#### [NEW] `lib/screens/ocr_review_screen.dart`
- Editable list of extracted values (user can fix mistakes)
- "Confirm & Save" button → saves as `HealthData` entries

#### [MODIFY] `lib/screens/data_category_screen.dart`
```
┌─────────────────────────────────┐
│   Select Data to Enter          │
│                                 │
│  ┌───────────────────────────┐  │
│  │ 📷 Scan Lab Report (NEW) │  │  ← NEW separate button
│  └───────────────────────────┘  │
│  ─── OR enter manually ───     │
│  ☐ Vitals                       │  ← Existing flow unchanged
...
1. **PDF**: Generate report → download → share via WhatsApp
2. **Trends**: Enter data across dates → view line charts → verify reference bands
3. **Dark Mode**: Toggle → all screens render correctly
4. **OCR**: Photo of lab report → values extracted → edit → save correctly
