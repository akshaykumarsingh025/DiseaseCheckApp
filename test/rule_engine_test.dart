...
import 'package:disease_check_app/engine/risk_result.dart';
...
void main() {
  ...
  group('RuleEngine - Full Pipeline (evaluateHealthData)', () {
    test('Mixed categories produce combined report', () {
      final entries = [
        _MockHealthData('fasting_glucose', 142),
        _MockHealthData('hba1c', 7.1),
        _MockHealthData('total_cholesterol', 260),
        _MockHealthData('ldl', 175),
        _MockHealthData('alt', 85),
      ...
      final diseases = results.map((r) => r.disease).toList();
